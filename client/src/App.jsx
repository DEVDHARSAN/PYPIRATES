...
function RegisterPage({ go, doRegister }) {
  const [form, setForm] = useState({ name: "", email: "", password: "", confirm: "", department: "", year: "1" });
  const [err, setErr] = useState(""); const [loading, setLoading] = useState(false);
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }));
  const submit = async (e) => {
    e.preventDefault(); setErr("");
    if (!form.department.trim()) return setErr("Please enter your department.");
    if (form.password.length < 6) return setErr("Password must be at least 6 characters.");
    if (form.password !== form.confirm) return setErr("Passwords do not match.");
    setLoading(true);
    const res = await doRegister({ ...form, department: form.department.trim() });
    setLoading(false);
    if (!res.ok) setErr(res.error);
  };
